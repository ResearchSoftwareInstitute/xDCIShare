...
def rep_res_bag_to_irods_user_zone(request, shortkey, *args, **kwargs):
    ...
    which is federated with hydroshare zone under the iRODS user account corresponding to a MyHPOM user. This function
    should only be called or exposed to be called from web interface when a corresponding iRODS user account on hydroshare
    user Zone exists. The purpose of this function is to allow MyHPOM resource bag that a MyHPOM user has access
    to be copied to MyHPOM user's iRODS space in MyHPOM user zone so that users can do analysis or computations on
    the resource
    Args:
...
def _send_email_on_group_membership_acceptance(membership_request):
    ...
    send_mail(subject="MyHPOM group membership",
              message=email_msg,
