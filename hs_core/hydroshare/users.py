...
def update_account(user, **kwargs):
    """
    Update an existing user within the MyHPOM system. The user calling this method must have
...
def get_resource_list(creator=None, group=None, user=None, owner=None, from_date=None,
    ...
    MyHPOM.getScienceMetadata() and MyHPOM.GetSystemMetadata() for every resource in the returned list.
